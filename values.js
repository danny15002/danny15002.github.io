...
const ValuesRankingApp = () => {
  ...
  const moveValue = (index, direction) => {
    ...
    switch (direction) {
      ...
      case 'up':
        ...
        break;
      case 'down':
        newIndex = Math.min(values.length - 1, index + 1);
        break;
      case 'bottom':
    ...
    if (newIndex !== index) {
      const [movedValue] = newValues.splice(index, 1);
  ...
  const getRowStyle = (index) => {
    if (index < 10) {
      return 'bg-yellow-100 border-yellow-300 shadow-md';
    } else if ((index + 1) % 5 === 0) {
    ...
    if (rank <= 10) {
      return (
        <div className="flex items-center">
          ...
          <span className="ml-2 text-sm text-yellow-600 font-medium">TOP 10</span>
        </div>
      );
    } else if (rank % 5 === 0) {
      return (
        <div className="flex items-center">
          <span className="text-lg font-semibold text-blue-600">#{rank}</span>
          <span className="ml-2 text-xs text-blue-500">MILESTONE</span>
        </div>
      );
    }
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-gray-50 min-h-screen">
      <div className="mb-8 text-center">
        <h1 className="text-3xl font-bold text-gray-800 mb-2">Personal Values Ranking</h1>
        <p className="text-gray-600">Rank these values by importance to you. Top 10 are highlighted in yellow, every 5th rank is marked in blue.</p>
      </div>
      ...
      <div className="space-y-2">
        {values.map((value, index) => (
          <div
            key={value.name}
            className={`p-4 rounded-lg border-2 transition-all duration-200 ${getRowStyle(index)}`}
          >
            <div className="flex items-center justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-bold text-gray-800 text-lg">{value.name}</h3>
              ...
              <div className="flex gap-1 ml-4">
                <button
                  onClick={() => moveValue(index, 'top')}
                  disabled={index === 0}
                  className="p-2 rounded bg-blue-500 text-white hover:bg-blue-600 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors"
                  title="Send to Top"
                >
                  <ChevronsUp size={16} />
                </button>
                <button
                  ...
                  disabled={index === 0}
                  className="p-2 rounded bg-green-500 text-white hover:bg-green-600 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors"
                  title="Move Up"
                >
                  <ChevronUp size={16} />
                </button>
                <button
                  ...
                  className="p-2 rounded bg-orange-500 text-white hover:bg-orange-600 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors"
                  title="Move Down"
                >
                  <ChevronDown size={16} />
                </button>
                <button
                  onClick={() => moveValue(index, 'bottom')}
                  disabled={index === values.length - 1}
                  className="p-2 rounded bg-red-500 text-white hover:bg-red-600 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors"
      ...
      <div className="mt-8 p-4 bg-white rounded-lg border border-gray-200">
        <h2 className="text-xl font-bold text-gray-800 mb-2">Your Top 10 Values:</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {values.slice(0, 10).map((value, index) => (
            <div key={value.name} className="flex items-center">
              <span className="font-bold text-yellow-600 mr-2">#{index + 1}</span>
...
export default ValuesRankingApp;